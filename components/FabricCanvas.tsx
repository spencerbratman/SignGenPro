...
const FabricCanvas: React.FC<FabricCanvasProps> = ({ color, setColor, isMobile }) => {
  ...
  const updatePathProperties = (canvas: fabric.Canvas, properties: Partial<fabric.IObjectOptions>) => {
  ...
  const toggleCleanup = () => {
    if (!cleanupEnabled) {
      ...
      setOriginalPaths(canvas?.getObjects().filter(obj => obj instanceof fabric.Path) as fabric.Path[]);
    } else {
  ...
  const applyCleanup = (intensity: number) => {
    if (canvas && fabric && originalPaths.length > 0) {
      const enhancedPaths = originalPaths.map(path => enhancePath(path, intensity));
      canvas.clear();
  ...
  const simplifyPath = (points: {x: number, y: number}[], tolerance: number): {x: number, y: number}[] => {
    ...
    for (let i = 1; i < points.length - 1; i++) {
      ...
      if (
        Math.pow(curr.x - prev.x, 2) + Math.pow(curr.y - prev.y, 2) > squaredTolerance ||
        Math.pow(next.x - curr.x, 2) + Math.pow(next.y - curr.y, 2) > squaredTolerance
      ) {
        newPoints.push(curr);
      }
    }
  ...
  return (
    <div className="flex flex-col">
      <canvas ref={canvasRef} className="border border-gray-300 rounded w-full" />
      <div className={`mt-4 ${isMobile ? 'flex flex-col space-y-2' : 'flex items-center space-x-4'}`}>
        <div className={`${isMobile ? 'w-full' : 'w-1/2'} flex items-center space-x-2`}>
          <label className="block text-sm font-medium text-gray-700">Brush Width</label>
          <input
            type="range"
            min="1"
            max="20"
            ...
            className="w-full"
          />
          <canvas ref={brushPreviewRef} width="50" height="50" className="border border-gray-300" />
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          {cleanupEnabled && (
            <>
              <button onClick={() => handleCleanupIntensityChange(-1)} className="px-2 py-1 bg-gray-200 rounded">-</button>
      ...
      <div className={`mt-4 ${isMobile ? 'flex flex-col space-y-2' : 'flex justify-between space-x-4'}`}>
        <button onClick={handleDownload} className="bg-blue-500 text-white px-4 py-2 rounded w-full">Download</button>
        <button onClick={handleUndo} className="bg-gray-200 text-gray-700 px-4 py-2 rounded w-full">Undo Last Stroke</button>
        <button onClick={handleClear} className="bg-gray-200 text-gray-700 px-4 py-2 rounded w-full">Clear</button>
      </div>
    </div>
  );
};
